...
class TelegramBot:
    ...
    async def _dispatch_photo_task(
        ...
        async def run_task() -> None:
            ...
            if len(paths) == 1:
                lines = [
                    "형님이 사진을 보냈습니다. 아래 경로의 이미지 파일을 Read 도구로 열어서 "
                    "내용을 확인하고 응답하세요.",
                    f"이미지 경로: {paths[0]}",
                ]
            else:
                lines = [
                    f"형님이 사진 {len(paths)}장을 한 번에(앨범) 보냈습니다. 아래 경로의 "
                    "이미지 파일들을 모두 Read 도구로 열어 함께 보고 하나의 응답으로 답하세요.",
                ]
                for i, p in enumerate(paths, 1):
                    lines.append(f"이미지 {i} 경로: {p}")
            if caption:
                lines.append(f"형님 캡션: {caption}")
    ...
    async def _handle_document_message(
        ...
        async def run_task() -> None:
            ...
            try:
                ...
                return
            lines = [
                "형님이 파일을 보냈습니다. 아래 경로의 파일을 Read 도구로 열어서 "
                "내용을 확인하고 응답하세요.",
                f"파일 경로: {dest}",
            ]
            if caption:
    ...
    async def _notify_outage_recovered(self, down_seconds: int) -> None:
        ...
        """
        # DGN-045 outage-recovered user notice disabled per 형님 request (2026-06-30):
