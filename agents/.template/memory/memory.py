...
def parse_markdown(path):
    ...
    if has_sep:
        ...
        def flush_section():
            ...
            section_buf.clear()
            # 김메탈식: 최상위 불릿("- ") = 청크 경계. 하위 들여쓰기/평문/빈줄은 현재 청크에 흡수.
...
def _parse_classify_output(stdout, n_items, valid_files):
    ...
    판정 = 주제파일명(예: about-siwon.md) | NEW:<라벨> | DROP.
...
def _run_classifier(items, topics):
    ...
    prompt = (
        ...
        f"{topic_lines}\n\n"
        "판정 규칙:\n"
        "- 항목이 위 주제파일 중 하나에 자연스럽게 속하면 그 파일명을 그대로 출력(예: about-siwon.md).\n"
...
def _emit_empty():
    """매칭 없어도 현재 시각 한 줄은 무조건 주입 + 성공.
    시각은 datetime이라 즉시 계산 — 턴을 막거나 느리게 하지 않는다."""
    try:
        out = {
            "hookSpecificOutput": {
                "hookEventName": "UserPromptSubmit",
                "additionalContext": _now_local_line(),
            }
        }
...
def cmd_hook(args):
    ...
    try:
        ...
        # 5) JSON으로 주입(stdout에 1회만). 시각 한 줄을 맨 앞에 prepend.
        out = {
            "hookSpecificOutput": {
                "hookEventName": "UserPromptSubmit",
                "additionalContext": _now_local_line() + "\n\n" + ctx,
...
def main():
    ...
    wp.add_argument("--file", default="inbox.md", help="적재 대상 md (기본 inbox.md — 주제 명확하면 identity/work-rules/routines/infra/about-siwon.md 지정)")
