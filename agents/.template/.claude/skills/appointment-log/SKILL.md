...
description: >-
  형님이 약속/일정을 잡거나 조회·수정하거나 참가자를 다룰 때 발동. "X랑 약속 잡았어",
  ...
  형님께 확인하는 "사람 해소 규칙"이다.
---

# appointment-log — 약속 관리

형님의 약속(appointments)을 lifekit.db에서 등록·수정·조회하고, 사람(persons)을
...
- 시간대는 항상 형님 기준 GMT+9(+09:00). 날짜는 YYYY-MM-DD, 시작/끝은 ISO(+09:00) 권장.
...
형님이 약속에 사람을 언급하면(본명이든 별명이든) 곧장 등록하지 말고 먼저 찾는다.
...
5. 끝나면 `appt-show <id>`로 약속+참가자 한 번 확인하고 형님께 요약 보고.
...
- 표·정렬 데이터는 코드블록(monospace)으로. 볼드(별표) 금지, 호칭은 형님, 존댓말.

## 경계
- 캘린더(Google Calendar) 반영이 필요하면 별도로 형님께 확인 후 처리(이 스킬은 로컬 DB만).
- 약속 삭제는 되돌리기 어려우니 형님 확인 후에만.
